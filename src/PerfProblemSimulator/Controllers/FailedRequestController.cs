...
namespace PerfProblemSimulator.Controllers
{
    ...
    /// </para>
    /// </remarks>
    [RoutePrefix("api/failedrequest")]
    public class FailedRequestController : ApiController
    {
        private readonly IFailedRequestService _failedRequestService;
        private static readonly Logger Logger = LogManager.GetCurrentClassLogger();

        public FailedRequestController(IFailedRequestService failedRequestService)
        {
        ...
        public IHttpActionResult Start([FromBody] FailedRequestRequest request)
        {
            var requestCount = request?.RequestCount ?? 10;

            Logger.Warn(
                "❌ Starting failed request simulation: Count={0}",
                requestCount);
        ...
        {
            Logger.Info("🛑 Stopping failed request simulation");
        ...
        [HttpGet]
        [Route("status")]
        [ResponseType(typeof(FailedRequestStatus))]
