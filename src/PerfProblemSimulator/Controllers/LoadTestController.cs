...
namespace PerfProblemSimulator.Controllers
{
    ...
    public class LoadTestController : ApiController
    {
        ...
        [HttpGet]
        [Route("")]
        [ResponseType(typeof(LoadTestResult))]
        public async Task<IHttpActionResult> ExecuteLoadTest(
            [FromUri] int workIterations = 200,
            [FromUri] int bufferSizeKb = 20000,
            [FromUri] int baselineDelayMs = 500,
            [FromUri] int softLimit = 25,
            [FromUri] int degradationFactor = 500,
            [FromUri] int errorAfter = 120,
            [FromUri] int errorPercent = 20,
            CancellationToken cancellationToken = default)
        {
            ...
            Logger.Debug(
                "Load test: WorkIterations={0}, BufferSizeKb={1}, BaselineDelayMs={2}, SoftLimit={3}, DegradationFactor={4}, ErrorAfter={5}s, ErrorPercent={6}%",
            ...
            catch (Exception ex)
            {
                ...
                Logger.Info(
                    "Load test threw expected exception: {0} - {1}",
                    ex.GetType().Name, ex.Message);
                
                return Content((HttpStatusCode)500, new 
                { 
        ...
        [HttpGet]
        [Route("stats")]
        [ResponseType(typeof(LoadTestStats))]
        public IHttpActionResult GetStats()
        {
            /*
             * SYNCHRONOUS ENDPOINT:
             * This returns IHttpActionResult (not Task<IHttpActionResult>) because it does
